namespace x360NANDManager.SPI {
    ...
    internal class ARMFlasher: FlasherOutput, ISPIFlasher, IDisposable {
        ...
        private ErrorCode ReadFromDevice(ref byte[] buf, int tries = 10) {
            ...
            while(totalread < buf.Length && tries > 0) {
                ...
                if(totalread > 0) {
                    ...
                    err = _reader.Read(tmp, 10000, out read);
                    if(read < buf.Length)
                        Buffer.BlockCopy(tmp, 0, buf, totalread, tmp.Length);
                }
                else
                    err = _reader.Read(buf, 10000, out read);
        ...
        protected ErrorCode WriteToDevice(byte[] buf, int tries = 10) {
            ...
            while(totalwrote < buf.Length && tries > 0) {
                ...
                if(totalwrote > 0) {
                    ...
                    err = _writer.Write(tmp, 10000, out wrote);
                }
                else
                    err = _writer.Write(buf, 10000, out wrote);
                if(err != ErrorCode.None && err != ErrorCode.IoTimedOut)
                    Main.SendDebug(String.Format("Error: {0}", err));
                totalwrote += wrote;
                tries--;
        ...
        ///     <exception cref="X360NANDManagerException">
        ///         If Device is not initalized, there is a fatal USB error or for
        ///         unsupported flashconfig types
        ///     </exception>
        ...
        ///         If
        ///         <paramref name="blockID" />
        ///         is greater then total blocks on device
        ///     </exception>
        /// </summary>
        /// <param name="blockID"> Block ID to erase </param>
        /// <param name="verboseLevel">
        ///     Specifies if you want alot of information on erase errors or just a erase error (default =
        ///     only print write error without details)
        /// </param>
        ...
        /// <summary>
        ///     Sends the erase command for BlockID:
        ///     <paramref name="startBlock" />
        ///     and onwards for
        ///     <paramref name="blockCount" />
        ///     <exception cref="X360NANDManagerException">If Device is not initalized or there is a fatal USB error</exception>
        ...
        /// <param name="verboseLevel">
        ///     Specifies if you want alot of information on write errors or just a write error (default =
        ///     only print write error without details)
        /// </param>
        ...
        /// <summary>
        ///     Writes buffer to device using specified write mode (
        ///     <paramref name="mode" />
        ///     ) starting at
        ///     <paramref name="startBlock" />
        ///     and writing untill the end of the the buffer or
        ///     <paramref name="blockCount" />
        /// </summary>
        /// <param name="startBlock"> Starting blockID </param>
        ...
        public void Write(uint startBlock, uint blockCount, byte[] data, SPIWriteModes mode = SPIWriteModes.None, int verboseLevel = 0) {
            ...
            if(eraseFirst) {
                ...
                if(!AbortRequested) {
                    sw.Stop();
                    UpdateStatus(string.Format("Erase Completed after {0:F0} Minutes and {1:F0} Seconds!{2}Device will be reset before writing...", sw.Elapsed.TotalMinutes, sw.Elapsed.Seconds,
                                               Environment.NewLine));
                    Reset();
                }
            }
            ...
            if(!AbortRequested && verify) {
                ...
                for(var block = startBlock; block <= lastBlock; block++) {
                    ...
                    var tmp = ReadBlock(block, false, verboseLevel);
        ...
        /// <summary>
        ///     Writes file to device using specified write mode (
        ///     <paramref name="mode" />
        ///     ) starting at
        ///     <paramref name="startBlock" />
        ///     and writing untill the end of the file or
        ///     <paramref name="blockCount" />
        /// </summary>
        /// <param name="startBlock"> Starting blockID </param>
        /// <param name="blockCount"> Block count (Small blocks!) if set to 0 full device/file write will be done </param>
                ...
                if(!AbortRequested) {
                    ...
                    for(var block = startBlock; block <= lastBlock; block++) {
                        ...
                        if(addSpare)
                            ...
                            CorrectSpareBlock(ref tmp, block, xConfig.MetaType);
                        TestMetaUtils(ref tmp, xConfig.MetaType);
                        WriteBlock(block, tmp, verboseLevel);
                        if(verify)
                            dataList.AddRange(tmp);
                    }
        ...
        /// <param name="startBlock"> Starting blockID </param>
        /// <param name="blockCount"> Block count (Small blocks!) if set to 0 full device/file write will be done </param>
        /// <param name="filestartblock"> Starting blockID for the file </param>
        /// <param name="file"> File to write </param>
        /// <param name="mode"> Write Mode to use (Default = None/RAW [Write data as is]) </param>
        /// <param name="verboseLevel">
        ///     Specifies if you want alot of information on write errors or just a write error (default =
        ///     only print write error without details)
        /// </param>
        public void Write(uint startBlock, uint blockCount, uint filestartblock, string file, SPIWriteModes mode = SPIWriteModes.None, int verboseLevel = 0)
        {
            CheckDeviceState();
            PrintARMVersion();
            AbortRequested = false;
            ...
            using (var br = OpenReader(file))
            {
                ...
                if (br.BaseStream.Length < (filestartblock * 0x4200) + (blockCount * 0x4200))
                    throw new ArgumentOutOfRangeException("filestartblock");
                br.BaseStream.Seek(filestartblock * 0x4200, SeekOrigin.Begin); // Seek to the block start

                #region Preperations

                var datablocks = (xConfig.GetFileBlockCount(file) - filestartblock);
                if(datablocks < blockCount || blockCount == 0)
                    blockCount = datablocks;
                var lastBlock = startBlock + blockCount - 1;
                var totalBlocks = lastBlock;
                ...
                UpdateStatus(string.Format("Verify after writing: {0}", verify ? "Enabled" : "Disabled"));
                UpdateStatus(string.Format("Write Mode: {0}", addSpare ? "Add Spare" : correctSpare ? "Correct Spare" : "RAW"));
                UpdateStatus(string.Format("Starting block: 0x{0:X} Starting Block (file): 0x{2:X} Last Block: 0x{1:X}", startBlock, lastBlock, filestartblock));
                UpdateStatus(string.Format("Source file: {0}", file));
        ...
        ///     Reads
        ///     <paramref name="blockID" />
        ///     using the block size specified by the flashconfig
        ///     <exception cref="X360NANDManagerException">If Device is not initalized or there is a fatal USB error</exception>
        ///     <exception cref="ArgumentOutOfRangeException">
        ///         If
        ///         <paramref name="blockID" />
        ...
        /// <param name="verboseLevel">
        ///     Specifies if you want alot of information on read errors or just a read error (default =
        ///     only print write error without details)
        /// </param>
        /// <returns> The block data </returns>
        public byte[] ReadBlock(uint blockID, bool zeroFillBadBlocks = true, int verboseLevel = 0) {
            CheckFlashState();
            if(blockID > _xcfg.SizeSmallBlocks)
                throw new ArgumentOutOfRangeException("blockID");
            var data = new byte[_xcfg.BlockReadSize];
        ...
        /// <summary>
        ///     Reads data from nand from block
        ///     <paramref name="startBlock" />
        ///     and onwards for
        ///     <paramref name="blockCount" />
        /// </summary>
        ...
        public byte[] Read(uint startBlock, uint blockCount, bool zeroFillBadBlocks = true, int verboseLevel = 0) {
            CheckDeviceState();
            PrintARMVersion();
            AbortRequested = false;
            var sw = Stopwatch.StartNew();
            ...
            for(var block = startBlock; block <= last; block++) {
                ...
                UpdateProgress(block, last);
                var tmp = ReadBlock(block, zeroFillBadBlocks, verboseLevel);
                datalist.AddRange(tmp);
            }
            DeInit();
            Release();
        ...
        ///     only print write error without details)
        /// </param>
        public void Read(uint startBlock, uint blockCount, string file, bool zeroFillBadBlocks = true, int verboseLevel = 0) {
            CheckDeviceState();
            AbortRequested = false;
            PrintARMVersion();
            var sw = Stopwatch.StartNew();
            ...
            using(var bw = OpenWriter(file)) {
                ...
                for(var block = startBlock; block <= last; block++) {
                    ...
                    UpdateProgress(block, last);
                    bw.Write(ReadBlock(block, zeroFillBadBlocks, verboseLevel));
                }
                DeInit();
                Release();
                if(AbortRequested)
        ...
        ///     only print write error without details)
        /// </param>
        public void Read(uint startBlock, uint blockCount, IEnumerable<string> files, bool zeroFillBadBlocks = true, int verboseLevel = 0) {
            CheckDeviceState();
            PrintARMVersion();
            AbortRequested = false;
            var sw = Stopwatch.StartNew();
            ...
            foreach(var file in files) {
                ...
                Read(startBlock, blockCount, file, zeroFillBadBlocks, verboseLevel);
                Reset();
            }
            if(AbortRequested)
                return;
        ...
        private static void TestMetaUtils(ref byte[] data, uint metaType) {
            for(uint i = 0; i < data.Length / 0x210; i++) {
                var mData = MetaUtils.GetMetaData(ref data, i);
                Main.SendDebug("LBA: 0x{0:X}", MetaUtils.GetLBA(ref mData, metaType));
                Main.SendDebug("Block Type: 0x{0:X}", MetaUtils.GetBlockType(ref mData, metaType));
                Main.SendDebug("FSSize: 0x{0:X}", MetaUtils.GetFSSize(ref mData, metaType));
                Main.SendDebug("FsFreePages: 0x{0:X}", MetaUtils.GetFsFreePages(ref mData, metaType));
                Main.SendDebug("FsSequence: 0x{0:X}", MetaUtils.GetFsSequence(ref mData, metaType));
                Main.SendDebug("BadBlock Marker: 0x{0:X}", MetaUtils.GetBadBlockMarker(ref mData, metaType));
            }
        }
