namespace x360NANDManager {
    ...
    internal sealed class XConfig {
        ...
        public readonly int SizeSmallBlocks;
        ...
        public XConfig(uint config) {
            ...
            SizeSmallBlocks = SizeBlocks * SmallBlocksCount;
        ...
        public void PrintXConfig(int verboselevel = 0) {
            if(Status != ErrorLevel.Success)
                return;
            if(verboselevel >= 0)
                Console.WriteLine("FlashConfig:       0x{0:X08}", Config);
            if(verboselevel >= 1) {
                Console.WriteLine("PageSize:          0x{0:X}", PageSize);
                Console.WriteLine("MetaSize:          0x{0:X}", MetaSize);
                Console.WriteLine("MetaType:          0x{0:X}", MetaType);
                Console.WriteLine("BlockSize:         0x{0:X}", BlockSize);
            }
            if(verboselevel >= 0)
                Console.WriteLine("SizeInBlocks:      0x{0:X}", SizeBlocks);
            if(verboselevel >= 2) {
                Console.WriteLine("SizeInSmallBlocks: 0x{0:X}", SizeSmallBlocks);
                Console.WriteLine("SmallBlocksCount:  0x{0:X}", SmallBlocksCount);
                Console.WriteLine("FileBlocks:        0x{0:X}", FSBlocks);
            }
            if(verboselevel >= 1)
            // ReSharper disable RedundantStringFormatCall
                Console.WriteLine(String.Format(new NumberFormatInfo { NumberGroupSeparator = " ", NumberDecimalDigits = 0 }, "SizeInBytes:       {0:N} B", SizeBytes));
            // ReSharper restore RedundantStringFormatCall
            if(verboselevel >= 0)
                Console.WriteLine("SizeInReadable:    {0}", Utils.GetSizeReadable(SizeBytes));
            if(verboselevel >= 3) {
                Console.WriteLine("ControllerType:    {0}", ControllerType);
